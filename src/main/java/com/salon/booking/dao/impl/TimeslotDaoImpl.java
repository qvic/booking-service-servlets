...
public class TimeslotDaoImpl extends AbstractCrudDaoImpl<TimeslotEntity> implements TimeslotDao {
    ...
    private static final String FIND_ALL_BETWEEN_DATES = "SELECT t.*, d.minutes FROM timeslot t INNER JOIN duration d ON t.duration_id = d.id WHERE t.date >= ? AND t.date < ? ORDER BY t.date, t.from_time";
    private static final String FIND_ALL_SAME_DAY = "SELECT t.*, d.minutes FROM timeslot t INNER JOIN duration d ON t.duration_id = d.id WHERE t.date = (SELECT date FROM timeslot WHERE id = ?) ORDER BY t.from_time";
